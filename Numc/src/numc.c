...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    ...
    if (!PyObject_TypeCheck(args, &Matrix61cType)){
        PyErr_SetString(PyExc_TypeError, "Not a Matrix61c.");
        return NULL;
    }
    ...
    int row_num1 = self->mat->rows;
//    int row_num2 = operand->mat->rows;
    int col_num1 = self->mat->cols;
//    int col_num2 = operand->mat->cols;

    if (allocate_matrix(&return_matrix, row_num1, col_num1)) {
//        PyErr_SetString(PyExc_TypeError, "Allocation to return_matrix fails.");
        return NULL;
    }

    if (add_matrix(return_matrix, self->mat, operand->mat)) {
        deallocate_matrix(return_matrix);
//        PyErr_SetString(PyExc_TypeError, "Incompatible dimensions / Null pointer.");
        return NULL;
    }

    final = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    final->mat = return_matrix;
    final->shape = PyTuple_Pack(2, PyLong_FromLong(row_num1), PyLong_FromLong(col_num1));
...
//    int col_num2 = operand->mat->cols;
    ...
    if (allocate_matrix(&return_matrix, row_num1, col_num1)) {
//        PyErr_SetString(PyExc_RuntimeError, "Allocation to return_matrix fails.");
        return NULL;
    }

    if (sub_matrix(return_matrix, self->mat, operand->mat)) {
        deallocate_matrix(return_matrix);
//        PyErr_SetString(PyExc_TypeError, "Incompatible dimensions / Null pointer.");
        return NULL;
    }
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
    /* TODO: YOUR CODE HERE */
    if (!PyObject_TypeCheck(args, &Matrix61cType)){
        PyErr_SetString(PyExc_TypeError, "args is not a Matrix61c.");
        return NULL;
    }
...
//    int col_num1 = self->mat->cols;
    int col_num2 = operand->mat->cols;
    if (allocate_matrix(&return_matrix, row_num1, col_num2)){
//        PyErr_SetString(PyExc_RuntimeError, "Allocation failed.");
        return NULL;
    }

    if (mul_matrix(return_matrix, self->mat, operand->mat)) {
        deallocate_matrix(return_matrix);
//        PyErr_SetString(PyExc_TypeError, "Incompatible dimensions / Null pointer.");
        return NULL;
    }
    ...
    final = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    final->mat = return_matrix;
    final->shape = PyTuple_Pack(2, PyLong_FromLong(row_num1), PyLong_FromLong(col_num2));
    return (PyObject *) final;
}
...
//        PyErr_SetString(PyExc_RuntimeError, "Allocation failed.");
    ...
    if (neg_matrix(return_matrix, self->mat)) {
        deallocate_matrix(return_matrix);
//        PyErr_SetString(PyExc_TypeError, "Incompatible dimensions / Null pointer.");
        return NULL;
    }
...
//        PyErr_SetString(PyExc_RuntimeError, "Allocation failed.");
    ...
    if (abs_matrix(return_matrix, self->mat)) {
        deallocate_matrix(return_matrix);
//        PyErr_SetString(PyExc_TypeError, "Incompatible dimensions / Null pointer.");
        return NULL;
    }
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    /* TODO: YOUR CODE HERE */
    if (!PyLong_Check(pow)) {
        PyErr_SetString(PyExc_TypeError, "Non-int power.");
        return NULL;
    }

    int power = (int) PyLong_AsLong(pow);
//    if (power < 0){
//        PyErr_SetString(PyExc_ValueError, "Negative power.");
//        return NULL;
//    }
...
//        PyErr_SetString(PyExc_RuntimeError, "Allocation failed.");
    ...
    if (pow_matrix(return_matrix, self->mat, power)) {
        deallocate_matrix(return_matrix);
//        PyErr_SetString(PyExc_TypeError, "Null pointer.");
        return NULL;
    }
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
    ...
    if (!PyTuple_Check(args) || (PyTuple_Size(args) != 3)) {
        PyErr_SetString(PyExc_TypeError, "args count != 3.");
        return NULL;
    }
    PyObject *args0 = PyTuple_GetItem(args, 0);
    PyObject *args1 = PyTuple_GetItem(args, 1);
    PyObject *args2 = PyTuple_GetItem(args, 2);
    if (!PyLong_Check(args0) || !PyLong_Check(args1)
        || (!PyLong_Check(args2) && !PyFloat_Check(args2))) {
        PyErr_SetString(PyExc_TypeError, "args format error.");
        return NULL;
    }

    int row_num = (int) PyLong_AsLong(args0);
    int col_num = (int) PyLong_AsLong(args1);
    double val = PyFloat_AsDouble(args2);

    if (row_num < 0 || col_num < 0
        || row_num >= self->mat->rows || col_num >= self->mat->cols) {
        PyErr_SetString(PyExc_IndexError, "Invalid row_num / col_num.");
        return NULL;
    }
    set(self->mat, row_num, col_num, val);
    return Py_None;
}
...
static PyObject *Matrix61c_get_value(Matrix61c *self, PyObject* args) {
    ...
    if (!PyTuple_Check(args) || (PyTuple_Size(args) != 2)) {
        PyErr_SetString(PyExc_TypeError, "args count != 2.");
        return NULL;
    }
    PyObject *args0 = PyTuple_GetItem(args, 0);
    PyObject *args1 = PyTuple_GetItem(args, 1);
    ...
    double val = get(self->mat, row_num, col_num);
...
static PyMethodDef Matrix61c_methods[] = {
    /* TODO: YOUR CODE HERE */
    {"get", (PyCFunction)Matrix61c_get_value, METH_VARARGS, "gets the value"},
    {"set", (PyCFunction)Matrix61c_set_value, METH_VARARGS, "sets the value"},
    {"add", (PyCFunction)Matrix61c_add, METH_VARARGS, "adds two matrices"},
    {"sub", (PyCFunction)Matrix61c_sub, METH_VARARGS, "subtracts matrix 2 from matrix 1"},
    {"neg", (PyCFunction)Matrix61c_neg, METH_VARARGS, "returned matrix = -1 * input matrix"},
    {"abs", (PyCFunction)Matrix61c_abs, METH_VARARGS, "abs. all values in the input matrix"},
    {"multi", (PyCFunction)Matrix61c_multiply, METH_VARARGS, "matrix 1 x matrix 2"},
    {"power", (PyCFunction)Matrix61c_pow, METH_VARARGS,"matrix ** power"},
    {NULL, NULL, 0, NULL}
};
